...
export default class extends BaseSchema {
  async up() {
    ...
    await this.db.rawQuery('CREATE INDEX IF NOT EXISTS companies_status_index ON companies (status)')
    await this.db.rawQuery(
      'CREATE INDEX IF NOT EXISTS companies_deleted_at_index ON companies (deleted_at)'
    )
    await this.db.rawQuery('CREATE INDEX IF NOT EXISTS companies_user_id_index ON companies (user_id)')

    await this.db.rawQuery(
      'CREATE INDEX IF NOT EXISTS halls_company_listing_index ON halls (company_id, deleted_at, created_at)'
    )
    await this.db.rawQuery('CREATE INDEX IF NOT EXISTS halls_city_index ON halls (city, deleted_at)')
    await this.db.rawQuery(
    ...
    await this.db.rawQuery('CREATE INDEX IF NOT EXISTS users_deleted_at_index ON users (deleted_at)')
