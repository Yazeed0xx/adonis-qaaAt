...
export default class extends BaseSchema {
  async up() {
    ...
    this.schema.raw(
      "CREATE UNIQUE INDEX company_invitations_pending_email_unique ON company_invitations (company_id, invited_email) WHERE status = 'pending' AND invited_email IS NOT NULL"
    )
    this.schema.raw(
      "CREATE UNIQUE INDEX company_invitations_pending_phone_unique ON company_invitations (company_id, invited_phone) WHERE status = 'pending' AND invited_phone IS NOT NULL"
    )
  }
